...
def use_warp_maps(origins, targets, fps, steps):
    ...
    # if video already exists change the name of the new one to avoid overwriting
    if num == 0:
        video = cv2.VideoWriter('morph/1.mp4', fourcc, fps, (ORIG_WIDTH, ORIG_HEIGHT))
        # change value of num to 1 to avoid overwriting
        os.remove("num.txt")
        num = 1
        # update num in num.txt
        with open('num.txt', 'w') as f:
            f.write(str(num))
            f.close()
    else:
        video = cv2.VideoWriter("morph/"+str(num+1)+".mp4", fourcc, fps, (ORIG_WIDTH, ORIG_HEIGHT))
        # change value of num to 0 to avoid overwriting
        os.remove("num.txt")
        num += 1
        # update num in num.txt
        with open('num.txt', 'w') as f:
